...
//     block.innerText = `${simpson.name} ${simpson.surname}. Возраст = ${simpson.age}  лет. ${simpson.info}`
...
//     let blockPhoto= document.createElement('img');
//     blockPhoto.src = simpson.photo;
//
...
for (const element of coursesArray) {
    ...
    blockTitle.innerText = element.title;

    let wrap = document.createElement('div');
    wrap.style.display = 'flex';

    let blockMonth = document.createElement('div');
    blockMonth.innerText = element.monthDuration;
    blockMonth.style.width = '100px';

    let blockHour = document.createElement('div');
    blockHour.innerText = element.hourDuration;
    blockHour.style.width = '100px';

    let blockModules = document.createElement('ul');
    for (const module of element.modules) {
        let item = document.createElement('li');
        item.innerText = module;
        blockModules.appendChild(item);
    }
    document.body.appendChild(blockTitle);
    document.body.appendChild(wrap);
    wrap.appendChild(blockMonth);
    wrap.appendChild(blockHour);
    document.body.appendChild(blockModules);
